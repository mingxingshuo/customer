...
FuUserSchema.statics = {
    fetch(id,tagId, codes, cb) {
        if (id) {
            return this.find({_id: {$lt: id},tagId: {$in: tagIds}, code: {$in: codes}, action_time: {$gt: Date.now() - 48 * 3600 * 1000}})
                .limit(50)
                .sort({'_id': -1})
                .exec(cb);
        } else {
    ...
    fetch_time(id,tagId, codes, pre, last, cb) {
        if (id) {
            return this.find({_id: {$lt: id},tagId: {$in: tagIds}, code: {$in: codes}, createAt: {$gte: pre, $lt: last}})
                .limit(50)
                .sort({'_id': -1})
                .exec(cb);
        } else {
